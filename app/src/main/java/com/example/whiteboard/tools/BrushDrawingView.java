...
public class BrushDrawingView extends View {
    ...
    private final Paint networkDrawPaint = new Paint();
    private Canvas mDrawCanvas;
    private boolean mBrushDrawMode;
    private boolean allowDraw = false;
    private boolean allowErase = false;

    private Path mPath;
    private Path networkPath;
    private float mTouchX, mTouchY;
    private float networkTouchX, networkTouchY;
...
//        }
    ...
    private void touchUp() {
        ...
        drawingListener.sendTouchUp(1,1);
    }

    public void networkMove(float from_x, float from_y, float to_x, float to_y) {
        networkPath.quadTo(networkTouchX, networkTouchY, (to_x + networkTouchX) / 2, (to_y+ networkTouchY) / 2);
        networkTouchX = to_x;
        networkTouchY = to_y;
    ...
    public void networkStart(float x, float y, float size, int color , String Xfermode) {
        networkDrawPaint.setColor(color);

        if(Xfermode.equals("CLEAR")) {
            networkDrawPaint.setStrokeWidth(DEFAULT_ERASER_SIZE);
    ...
    public void networkUp() {
        mDrawCanvas.drawPath(networkPath, networkDrawPaint);
        // kill this so we don't double draw
    ...
    public void setAllowErase(boolean allowErase) {
